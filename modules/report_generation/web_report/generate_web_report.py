    ...
    def generate_main_content(self):
        ...
        for module_name in self.audit_result.keys():
            ...
            for test_name in self.audit_result[module_name].keys():
                ...
                temp_test_content =f"""
                <div id="{module_name.replace(' ','').replace('.','').replace('(','').replace(')','')}_{test_name.replace(' ','').replace('.','').replace('(','').replace(')','')}_content"><a class="go_back" href="#main_table"><h5>Web Report</a> > <a class="go_back" href="#{module_name.replace(' ','').replace('.','').replace('(','').replace(')','')}_content">{module_name}</a> > {test_name}</h5><hr /><br />{test_table}</div>
            """
